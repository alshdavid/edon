...
macro_rules! impl_object_methods {
  ($js_value:ident) => {
    impl $js_value {
      ...
      pub fn get_property_unchecked<K, T>(
        &self,
        key: K,
      ) -> Result<T>
      where
      ...
      pub fn get_named_property_unchecked<T>(
        &self,
        name: &str,
      ) -> Result<T>
      where
      ...
      pub fn has_named_property<N: AsRef<str>>(
        &self,
        name: N,
      ) -> Result<bool> {
        let mut result = false;
      ...
      pub fn get_element_unchecked<T>(
        &self,
        index: u32,
      ) -> Result<T>
      where
